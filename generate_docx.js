...
const picOnsite  = getArg('--pic-onsite') || '';
...
const FONT_SIZE_BODY  = 20;   // 10pt (half-points)
const FONT_SIZE_HDR   = 20;   // 10pt
  ...
  return new TableRow({
    ...
    children: hdrs.map((h, i) =>
      makeCell(
        ...
        { width: COL_WIDTHS[i], vAlign: VerticalAlign.CENTER }
      )
    ),
  });
}
...
function infoTable() {
  ...
  return new Table({
    ...
    rows: [
      new TableRow({ children: [iCell('Vendor Name',     w[0]), iCell(':', w[1]), iCell('PT Code Development Indonesia', w[2]), iCell('Supervisor',  w[3]), iCell(':', w[4]), iCell('Mochamad Firman', w[5])] }),
      new TableRow({ children: [iCell('Consultant name', w[0]), iCell(':', w[1]), iCell(employee_name, w[2]), iCell('PIC Onsite', w[3]), iCell(':', w[4]), iCell(picOnsite, w[5])] }),
  ...
  return new Table({
    ...
    rows: [
      new TableRow({ children: [
        ...
        sCell(['Supervisor'], sigW, [0], AlignmentType.CENTER),
        sCell(['PIC Onsite'], rem, [0], AlignmentType.CENTER)
      ] }),
      new TableRow({ children: [
        sCell([employee_name], sigW, [], AlignmentType.CENTER),
        sCell(['Mochamad Firman'], sigW, [], AlignmentType.CENTER),
        sCell([picOnsite], rem, [], AlignmentType.CENTER)
      ] }),
      new TableRow({ children: [
        sCell(['', '', '', '', ''], sigW),
        sCell(['', '', '', '', ''], sigW),
